...
function Item(props) {
  ...
  useEffect(() => {
    const fetchlike = async () => {
      try {
        const res = await axios.get(`${API_URL}/prod/like/check`, {
          params: { userid: getCookie("login"), prodid: id },
        });
        setLike(res.data);
      } catch (error) {
  ...
  useEffect(() => {
    const getlikecount = async () => {
      try {
        const res = await axios.get(`${API_URL}/prod/like/getlike`, {
          params: { prodid: id },
        });
  ...
  const toggleLike = async () => {
    if (getCookie("login")) {
      try {
        if (like) {
          // 찜 삭제
          await axios.delete(`${API_URL}/prod/like/remove`, {
            params: { userid: getCookie("login"), prodid: id },
          });
        } else {
          // 찜 추가
          await axios.post(`${API_URL}/prod/like/add`, {
                  ...
                  <div className="KJH_item_info_status_info_share">
                    ...
                    <div>
                      <div className="KJH_item_info_share_section">
                        ...
                        />
                        <div
                          className={`KJH_item_info_shear_link fadeInOut ${
